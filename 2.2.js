...
for (let j = 0; j < 100; j++) {
  for (let z = 0; z < 100; z++) {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      switch (+values[i]) {
        case 1:
          newNumb = +values[+values[i + 1]] + +values[+values[i + 2]];
          values[+values[i + 3]] = newNumb;
          i += 3;
          break;
        case 2:
          newNumb = +values[+values[i + 1]] * +values[+values[i + 2]];
    ...
    if (values[0] == 19690720) {
      console.log(`First value: ${j}`);
      console.log(`Second value: ${z}`);
      console.log(`Solution: ${100 * j + z}`);
    }
  }
}
