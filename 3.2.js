// Advent of code 2019, day 3.1, faster solution but more math
...
for (let i = 0; i < values1.length; i++) {
  let letter1 = values1[i][0];
  let number1 = +values1[i].slice(1, values1[i].length);
  position2 = [0, 0];
  switch (letter1) {
    case "U":
      position1[1] += number1;
  ...
  for (let j = 0; j < values2.length; j++) {
    let letter2 = values2[j][0];
    let number2 = +values2[j].slice(1, values2[j].length);
    switch (letter2) {
      case "U":
        position2[1] += number2;
        break;
